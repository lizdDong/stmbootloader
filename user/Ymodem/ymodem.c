...
int32_t Receive_Byte(uint8_t *c, uint32_t timeout)
{
    while(timeout-- > 0)
    {
        if(USART_GetFlagStatus(COM_PORT, USART_FLAG_RXNE) != RESET)
        {
            *c = (uint8_t)COM_PORT->DR;
            return 0;
        }
    }
    return -1;
...
/**
  ...
  * @retval 0: Byte sent
  */
uint32_t Send_String(char *pstr)
{
    while(*pstr != '\0')
    {
        Send_Byte(*pstr++);
                            ...
                            else
                            {
                                if(packets_received == 0) //文件信息(首包)
                                {
                                    ...
                                    if(packet_data[PACKET_HEADER] != 0) //文件名字
                                    {
                                        ...
                                        file_size[i++] = '\0';
                                        Str2Int(file_size, (uint32_t *)&size);
                                        ...
                                        /* Define the number of page to be erased */
                                        {
                                            ...
                                            {
                                                if(FLASH_ErasePage(FlashDestination + (PageSize * i)) != FLASH_COMPLETE)
                                                {
                                                    /* End session */
                                                    Send_Byte(CA);
                                                    Send_Byte(CA);
